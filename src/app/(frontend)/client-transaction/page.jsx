...
const ClientTransaction = () => {
    ...
    useEffect(() => {
        ...
        const activeClients = storedAccounts.filter((account) => account.status === true) // Only include active clients
        const activeClientsNames = activeClients.map((account) => account.name); // Keep only the name
        setClients(activeClientsNames);
    }, []);
        ...
        <Header />
            ...
            <Container className="mt-3 px-3 px-sm-4 py-4 bg-light rounded-4 shadow-sm w-100 w-md-75 mx-auto">
                ...
                <Form onSubmit={handleSubmit}>
                    ...
                    <Row className="mt-3">
                        <Col sm={6}>
                            <Form.Label className="fw-bold">Total Credits</Form.Label>
                            <Form.Control value={getTotalCredit()} readOnly className="bg-white" />
                        </Col>
                        <Col sm={6}>
                            <Form.Label className="fw-bold">Remaining Amount</Form.Label>
